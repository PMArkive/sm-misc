...
// New joystick callback
public void OnJoystickRetrieved(QueryCookie cookie, int client, ConVarQueryResult result, const char[] cvarName, const char[] cvarValue)
{
    bool joyStick = (0.0 <= StringToFloat(cvarValue) < 1.0) ? false : true;
    if (joyStick)
    {
        KickClient(client, "Joystick is not allowed");
        AnticheatLog(client, "was kicked for having joystick enabled.");
    }
    else
    {
        g_JoyStick[client] = false;
...
float GetAverage(int[] array, int size, bool countZeroes = true)
{
	...
	float float_average = float(total) / float(size);
    ...
    if(g_iKeySwitch_CurrentFrame[client][btype] == 0)
    {
        ...
        for(int idx; idx < MAX_FRAMES_KEYSWITCH; idx++)
        {
            if(g_bKeySwitch_IsRecorded[client][btype][idx] == true)
            {
                ...
                if(btype == BT_Key && g_iKeySwitch_Stats[client][KeySwitchData_Difference][BT_Key][idx] >= 0)
                {
                    positiveCount++;
                }
        ...
        if(sd <= 0.25)
        {
            ...
            AnticheatLog(client, "key switch %d, avg: %.2f, dev: %.2f, p: %.2f％, Timing: %.1f", btype, mean, sd, positivePct * 100, timingPct * 100);
        }
    }
}
	...
	*/
	...
	if(g_iIllegalSidemoveCount[client] == 0)
	{
		if(g_iLastIllegalSidemoveCount[client] >= 10)
		{
			...
			if((float(g_iYawChangeCount[client]) / float(g_iLastIllegalSidemoveCount[client])) > 0.3) // Joystick now autokicks
			{
				bBan = true;
			}

			AnticheatLog(client, "has invalid consecutive movement values, (YawChanges = %d/%d) - %s", g_iYawChangeCount[client], g_iLastIllegalSidemoveCount[client], bBan?"BAN":"SUSPECT");
